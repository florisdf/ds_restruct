...
# Count number of times a special character is inside a generic string
def count_special_chars(string):
    return {name: len(re.findall(r'([^\\]|^){}'.format(char)))
...
parser.add_argument('--input_format', help='The input format.\
                    Use "{camera_id}" for camera-id, "{person_id}" for person-id, "{image_id}" for image-id, "{deeper_folder}" for a deeper folder level and "{dont_care}" for a don\'t care.  Use "{escape}" to escape any of the special characters.\
                    ...
                    If no camera-id (or person-id) is specified, the images will be assumed to all have the same camera-id(resp. person-id).'
...
print(count_special_chars(input_format))
